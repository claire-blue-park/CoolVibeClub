...
struct NearByView: View {
  ...
  @StateObject private var store = NearByStore()
      ...
      ZStack {
        VStack(spacing: 0) {
          ...
          if locationService.authorizationStatus == .authorized {
            DistanceSliderView { distance in
              store.send(.setSelectedDistance(distance))
            }
          ...
          ScrollView(.vertical) {
            LazyVStack(spacing: 0) {
              ActivityPostSection(
                ...
                hasMorePosts: store.state.hasMorePosts,
                onLoadMore: {
                  store.send(.loadMorePosts(location: locationService.currentLocation))
                },
                onRefresh: {
                  store.send(.refreshPosts(location: locationService.currentLocation))
                }
              )
//              .padding(.top, 16)
            }
        ...
        VStack {
          ...
          HStack {
            Spacer()
            WritingButton {
              store.send(.setShowEditView(true))
            }
            .padding(.trailing, 20)
      ...
      .task {
        ...
        if locationService.authorizationStatus == .authorized {
          ...
          // 초기 포스트 로드
          store.send(.loadPosts(location: locationService.currentLocation))
        }
      }
      .onAppear {
        ...
        locationService.onLocationAuthorized = { location in
          store.send(.loadPosts(location: location))
        }
      }
      ...
      .sheet(isPresented: .init(
        get: { store.state.showEditView },
        set: { store.send(.setShowEditView($0)) }
      )) {
        EditView()
      }
    }
